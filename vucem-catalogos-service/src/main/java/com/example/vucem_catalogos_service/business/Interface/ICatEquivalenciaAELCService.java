package com.example.vucem_catalogos_service.business.Interface;

import com.example.vucem_catalogos_service.model.dto.CatEquivalenciaAelcDTO;
import com.example.vucem_catalogos_service.model.dto.PageResponseDTO;
import org.springframework.data.domain.Pageable;

import java.time.Instant;

public interface ICatEquivalenciaAELCService {

    PageResponseDTO<CatEquivalenciaAelcDTO> list(String search, Pageable pageable);

    CatEquivalenciaAelcDTO findById(Instant fecIniVigencia, String cveMoneda);

    CatEquivalenciaAelcDTO create(CatEquivalenciaAelcDTO dto);

    CatEquivalenciaAelcDTO update(Instant fecIniVigencia, String cveMoneda, CatEquivalenciaAelcDTO dto);
}
